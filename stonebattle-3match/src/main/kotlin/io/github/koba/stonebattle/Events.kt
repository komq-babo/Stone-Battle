...
import io.github.koba.stonebattle.Events.Companion.team1out
import io.github.koba.stonebattle.Events.Companion.team2out
import io.github.koba.stonebattle.Events.Companion.team3out
...
import io.github.koba.stonebattle.KommandStone.team3
import io.github.koba.stonebattle.KommandStone.team3bar
...
class Events : Listener {
    ...
    companion object {
        ...
        var team1out: Boolean = false
        var team2out: Boolean = false
        var team3out: Boolean = false
    }
    ...
    fun onProjectileHit(event: ProjectileHitEvent) {
        ...
        if (event.hitBlock is Block) {
            ...
            if (block.type == Material.COBBLESTONE) {
            ...
            } else if (block.location == getInstance().config.getLocation("${team1}.core") && !team1out) {
                if (!game) return
                ...
                if (team1bar!!.progress <= 0.005) {
                    ...
                    for (player in Bukkit.getOnlinePlayers()) {
                        player.sendMessage("$team1 팀의 코어가 파괴되었습니다.")
                        if (Bukkit.getScoreboardManager().mainScoreboard.getTeam(team1.toString())!!
            ...
            } else if (block.location == getInstance().config.getLocation("${team2}.core") && !team2out) {
                if (!game) return
                val shooter = event.entity.shooter as Player
                ...
                if (team2bar!!.progress <= 0.005) {
                    team2out = true
                    for (player in Bukkit.getOnlinePlayers()) {
                        player.sendMessage("$team2 팀의 코어가 파괴되었습니다.")
                        if (Bukkit.getScoreboardManager().mainScoreboard.getTeam(team2.toString())!!
                                .hasEntry(player.name)
                        ) {
                            player.gameMode = GameMode.SPECTATOR
            ...
            } else if (block.location == getInstance().config.getLocation("${team3}.core") && !team3out) {
                if (!game) return
                val shooter = event.entity.shooter as Player
                if (Bukkit.getScoreboardManager().mainScoreboard.getTeam(team3.toString())!!
                        .hasEntry(shooter.name)
                ) return

                if (team3bar!!.progress <= 0.005) {
                    team3out = true
                    for (player in Bukkit.getOnlinePlayers()) {
                        player.sendMessage("$team3 팀의 코어가 파괴되었습니다.")
                        if (Bukkit.getScoreboardManager().mainScoreboard.getTeam(team3.toString())!!
                                .hasEntry(player.name)
                        ) {
                            player.gameMode = GameMode.SPECTATOR
                            ...
                            team3bar?.removePlayer(player)
                        }
                    }
                    end()
                    return
                }
                team3bar!!.progress -= 0.005
                val afterProgress = team3bar?.progress
                shooter.playSound(shooter.location, Sound.ENTITY_ARROW_HIT_PLAYER, 0.5f, 1f)
                team3bar?.addPlayer(shooter)

                Bukkit.getScheduler().scheduleSyncDelayedTask(getInstance(), {
                    if (team3bar?.progress == afterProgress) {
                        team3bar?.removePlayer(shooter)
                    }
                }, 100)
    ...
    fun onBlockBreak(event: BlockBreakEvent) {
        ...
        val count = max(1, sqrt(Random.nextInt(64).toDouble()).toInt())
...
private fun end() {
    ...
    if (team1out && team2out) {
        ...
        team3out = false
        for (player in Bukkit.getOnlinePlayers()) {
            player.sendTitle("게임 종료!", team3, 10, 100, 10)
            player.gameMode = GameMode.SPECTATOR
    ...
    } else if (team2out && team3out) {
        team1out = false
        team2out = false
        team3out = false
        for (player in Bukkit.getOnlinePlayers()) {
            player.sendTitle("게임 종료!", team1, 10, 100, 10)
            player.gameMode = GameMode.SPECTATOR
            Bukkit.getScheduler().cancelTasks(getInstance())
            team1bar?.removePlayer(player)
            team2bar?.removePlayer(player)
    ...
    } else if (team1out && team3out) {
        team1out = false
        team2out = false
        team3out = false
        for (player in Bukkit.getOnlinePlayers()) {
            player.sendTitle("게임 종료!", team2, 10, 100, 10)
            player.gameMode = GameMode.SPECTATOR
            Bukkit.getScheduler().cancelTasks(getInstance())
            team1bar?.removePlayer(player)
            team2bar?.removePlayer(player)
